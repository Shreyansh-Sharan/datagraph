...
class Registry:
    ...
    def create_version(self, domain_id: UUID, *, actor: str) -> DomainVersion:
        with self._cur() as cur:
            ...
            src = latest or {}
            row = cur.execute(
                "INSERT INTO domain_versions (domain_id, version, status, ontology_ttl, mapping, r2rml_ttl) "
                "VALUES (%s, %s, 'draft', %s, %s, %s) RETURNING *",
                (domain_id, (latest["version"] + 1) if latest else 1, src.get("ontology_ttl"),
                 Jsonb(src["mapping"]) if src.get("mapping") is not None else None, src.get("r2rml_ttl"))).fetchone()
    ...
    def update_content(self, version_id: UUID, *, actor: str, ontology_ttl: str | None = None,
                       mapping: dict | None = None, r2rml_ttl: str | None = None) -> DomainVersion:
        with self._cur() as cur:
            ...
            sets, params, changed = [], [], []
            for col, val in (("ontology_ttl", ontology_ttl), ("mapping", mapping), ("r2rml_ttl", r2rml_ttl)):
                if val is not None:
                    sets.append(f"{col} = %s")
                    params.append(Jsonb(val) if col == "mapping" else val)
                    changed.append(col)
            if not sets:
                return _version(row)
            row = cur.execute(
