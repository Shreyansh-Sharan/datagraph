              ...
              return (
                <div key={sc.id} role="treeitem" aria-expanded={open} aria-label={sc.label}>
                  ...
                  <div className="row" style={{ gap: 6, padding: "8px 10px 8px 8px", borderTop: "1px solid var(--line-2)", background: sc.id === schema ? "var(--surface-2)" : undefined }}>
                    ...
                    <a href="#" className="mono" style={{ flex: 1, minWidth: 0, fontSize: 12, fontWeight: 700, color: "var(--ink)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={sc.label} onClick={e => { e.preventDefault(); setExpanded(x => ({ ...x, [sc.id]: true })); if (all[0]) setParams({ schema: sc.id, table: all[0].name }); }}>{sc.label.includes(".") ? sc.label.split(".").pop() : sc.label}</a>
