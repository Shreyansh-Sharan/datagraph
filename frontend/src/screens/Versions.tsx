...
export function Versions() {
  const { api, say, rank, can } = useApp();
  const { domain, versions, version: current, setVersion, patch } = useDomain();
  const go = useGo();
  const audit = useLoad(() => api.audit(domain.name), [domain.name]);
  ...
  const transition = async (v: number, to: VersionStatus | "active", msg: string) => { patch(await api.transition(domain.name, v, to)); say(msg); };
  const createDraft = async (from?: number) => { if (draftExists) { say(`Draft v${versions[0].version} already exists · open it`); setVersion(versions[0].version); return; } patch(await api.createDraft(domain.name, from)); say(`Draft v${(versions[0]?.version || 0) + 1} created`); };
  ...
  const actionsFor = (v: VersionInfo) => {
    ...
    const rv = v.review;
    if (v.status === "draft") { if (rank >= 1) { acts.push({ label: "Submit for review", kind: "primary", go: () => transition(v.version, "in_review", `v${v.version} submitted for review`) }); acts.push({ label: "Build", kind: "default", go: () => go("build", { v: v.version }) }); } acts.push({ label: "Open", kind: "default", go: () => { setVersion(v.version); go("overview", { v: v.version }); } }); if (rank >= 1) acts.push({ label: "Delete draft", kind: "danger", go: () => say("Type the version number to confirm deletion") }); }
    if (v.status === "in_review") { if (rank >= 2) { acts.push({ label: "Approve", kind: "primary", go: () => say(`Approved v${v.version} · ${rv ? rv.approved + 1 : 1} of ${domain.quorum}`) }); acts.push({ label: "Reject with comment", kind: "default", go: () => transition(v.version, "draft", `v${v.version} sent back to draft`) }); acts.push({ label: rv && rv.approved >= rv.quorum ? "Publish" : "Publish (quorum not met)", kind: "default", go: () => rv && rv.approved >= rv.quorum ? transition(v.version, "published", `v${v.version} published`) : say(`Needs ${(rv?.quorum ?? domain.quorum) - (rv?.approved ?? 0)} more approval(s)`) }); } acts.push({ label: "Open", kind: "default", go: () => go("overview", { v: v.version }) }); }
    if (v.status === "published") { if (!v.active && rank >= 2) acts.push({ label: "Set active", kind: "primary", go: () => transition(v.version, "active", `v${v.version} is now served to MCP and GraphQL`) }); if (rank >= 1) acts.push({ label: "Create draft from this", kind: "default", go: () => createDraft(v.version) }); acts.push({ label: "Explore graph", kind: "default", go: () => go("explore", { v: v.version }) }); if (rank >= 2 && !v.active) acts.push({ label: "Archive", kind: "danger", go: () => transition(v.version, "archived", `v${v.version} archived`) }); }
    if (v.status === "archived") { if (rank >= 1) acts.push({ label: "Restore as draft", kind: "default", go: () => createDraft(v.version) }); acts.push({ label: "Export bundle", kind: "default", go: () => say(`Exporting ${domain.name} v${v.version} bundle`) }); }
    return acts;
  };
  ...
  return (
    <>
      <div className="page-head">
        ...
        <div className="actions"><Button active={cmpOpen} onClick={() => setCmpOpen(o => !o)}>Compare</Button>{can("builder") && <Button variant="primary" onClick={() => createDraft()}>{draftExists ? "Open current draft" : "Create draft"}</Button>}</div>
      </div>
      ...
      {cmpOpen && versions.length > 0 && (
        <Card flush style={{ borderColor: "var(--blue-border)", marginBottom: 20 }}>
          <div className="row" style={{ gap: 12, padding: "14px 20px", borderBottom: "1px solid var(--line)", background: "var(--blue-soft)" }}>
            ...
            <span className="spacer" /><Button size="sm" onClick={() => say("Diff exported")}>Export diff</Button>
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1.5fr) minmax(0,1fr)", gap: 20, alignItems: "start" }}>
        <div style={{ position: "relative", paddingLeft: 28 }}>
          ...
          {versions.map((v, i) => {
            ...
            const flags = [[`${st.classes} classes`, true], [`mapping ${v.mappingPct ?? "—"}%`, v.mappingPct === 100], [`${st.rules} rules`, st.rules > 0], [`${st.constraints} constraints`, st.constraints > 0], [v.lastBuild.replace(" · ", " "), v.lastBuild.startsWith("succeeded")]] as [string, boolean][];
            return (
              <div key={v.version} style={{ position: "relative", marginBottom: 14 }}>
                ...
                <Card style={{ padding: "16px 20px", borderColor: v.version === current?.version ? BLUE : undefined }}>
                  <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                    ...
                    <span className="muted-2 small" style={{ marginLeft: "auto" }}>{v.created} · {v.by}</span>
                  </div>
                  ...
                  {rv && <div className="row" style={{ marginTop: 12, gap: 10, fontSize: 12 }}><span className="muted">Review</span><div className="row" style={{ gap: 3, width: 120 }}>{Array.from({ length: rv.quorum }, (_, k) => <i key={k} style={{ flex: 1, height: 6, borderRadius: 3, background: k < rv.approved ? BLUE : "var(--line)" }} />)}</div><span style={{ fontWeight: 600, color: "var(--blue-dark)" }}>{rv.approved} of {rv.quorum}</span><span className="muted-2">{rv.rows.map(r => `${r.who} ${r.state === "approved" ? "✓" : "·"}`).join("  ")}</span></div>}
                  {v.lease && <div className="muted small" style={{ marginTop: 10 }}>Lease held by <strong style={{ color: "var(--ink)", fontWeight: 600 }}>{v.lease.holder}</strong>, expires in {v.lease.expires}</div>}
          ...
          {versions.length === 0 && <Card className="dashed"><div style={{ fontSize: 16, fontWeight: 800, marginBottom: 6 }}>No version yet</div><p className="muted" style={{ marginBottom: 14 }}>Create the first draft for {domain.name}.</p><Button variant="primary" onClick={() => createDraft()}>Create a draft version</Button></Card>}
        </div>
        <div className="grid">
          ...
          <Card>
            <div className="row between" style={{ alignItems: "baseline", marginBottom: 6 }}><h2 className="h2">Audit log</h2><a href="#" className="small" style={{ fontWeight: 600 }} onClick={e => { e.preventDefault(); say("Audit log exported"); }}>Export</a></div>
