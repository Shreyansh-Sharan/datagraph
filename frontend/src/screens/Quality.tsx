...
import { Button, Card, Dot, Skeleton, Tabs } from "@/components/ui";
import { useApp, useLoad } from "@/state/app";
import { useDomain, useGo, useParam } from "@/state/domain";
import { relTime } from "@/api/format";
import type { DqTableSummary } from "@/api";
  ...
  return (
    <div style={{ marginTop: 16 }}>
      <div className="row between" style={{ marginBottom: 12 }}><p className="muted">Constraints over the built graph, checked in SQL; sample violators link into Explore.</p>
        <div className="actions">{editable && <Button onClick={() => say("Derived 3 constraints from ontology restrictions")}>Derive from ontology</Button>}<Button variant="primary" onClick={() => { say("Checks run · 15 violations, 3 warnings"); rows.reload(); }}>Run checks</Button></div></div>
      <Card flush>
